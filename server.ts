...
// Read Firebase configuration dynamically in a Node-safe way
...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    // SPA Wildcard Route (supports other routes)
    app.get("*", (req, res) => {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Server fully functional with Firestore and running at http://0.0.0.0:${PORT}`);
  });
}
